% Metas.UncLib.Matlab.DistProp V2.9.2
...
classdef DistProp
    ...
    methods
            ...
            switch nargin
                ...
                case 2
                    ...
                    elseif isa(varargin{1}, 'Metas.UncLib.Core.Unc.Distribution') && isa(varargin{2}, 'char')
                        obj.NetObject = Metas.UncLib.DistProp.UncNumber(varargin{1}, UncInputId(), sprintf(varargin{2}));
                    else
                        error('Wrong type of input arguments')
                    end
                case 3
                    ...
                    elseif isa(varargin{1}, 'Metas.UncLib.Core.Unc.Distribution') && isa(varargin{2}, 'Metas.UncLib.Core.Unc.InputId') && isa(varargin{3}, 'char')
                        obj.NetObject = Metas.UncLib.DistProp.UncNumber(varargin{1}, varargin{2}, sprintf(varargin{3}));
                    else
                        error('Wrong type of input arguments')
                    end
                case 4
        ...
        function z = ellippi(x,y)
            ...
            if ~x.IsArray && ~y.IsArray
                z = DistProp(x.NetObject.Ellippi(y.NetObject));
            elseif x.IsArray && ~y.IsArray
                y = y.*ones(size(x));
                z = ellippi(x,y);
            elseif ~x.IsArray && y.IsArray
        ...
        function g = gradient(f, x)
            ...
            x = double(x(:));
            s = size(f);
            numlib = DistProp.NumLib2(f.IsComplex);
            fm = DistProp.Convert2UncArray(f);
            gm = numlib.Gradient(fm, x);
            g = DistProp.Convert2DistProp(gm);
            g = reshape(g, s);
        end
