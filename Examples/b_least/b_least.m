...
function [b, fweighted, res] = b_least(py, px, b_func)
% b_least fits the coefficients b of the fit function b_func using the
% points py and px.
...
xOpt2 = optimizer(objective, xOptStart2, pOpt, false, [], [], [], 1e-9);
xOpt = xOpt2.*xOptScale;
b = xOpt(n+1:end);
% residual
fweighted = objective(double(xOpt2), double(pOpt));
